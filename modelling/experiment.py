...
def model_predict(X_train, X_test, y_train, y_test, parameters, model):
    ...
    """
    grid = GridSearchCV(model, parameters, scoring="neg_mean_squared_error", cv=5, verbose=1, n_jobs=n_job)
    ...
    error_values = (y_test.values.flatten() - y_pred)
    error = mean_squared_error(y_test, y_pred) * len(y_test)
    return error, mean_error, std, len(y_test), error_values


def cluster_predict(X_train, X_test, y_train, y_test, parameters, model, show_info = True, num_cluster=num_cluster):
    output_dict = {}
